...
>>>>>>> 6f8fab9ee20e9c26d707603155d0e2565f492358
    ...
    `}}customElements.define("chats-page",eo);const ei="/live-chat",ea=new X(document.querySelector(".root"));console.log("se ha iniciado el router."),console.log("la abse url del router es: ",ea.baseUrl),ea.setRoutes([{path:ei+"/",component:"home-page"},{path:ei+"/chatrooms",component:"chatrooms-page"},{path:ei+"/chat/:id",component:"chats-page"}]);const es="https://backend-live-chat.onrender.com";function ec(){fetch(es+"/checkrooms",{method:"PUT",headers:{"Content-Type":"application/json"},body:JSON.stringify({username:localStorage.getItem("name"),color:localStorage.getItem("color")})}).then(e=>e.json()).then(e=>{console.log(e)})}async function el(e){let t=localStorage.getItem("name"),n=localStorage.getItem("color"),r=(await ep(e)).idFull;fetch(es+"/chat/"+e+"/addparticipant",{method:"PUT",headers:{"Content-Type":"application/json"},body:JSON.stringify({name:t,color:n,idShort:e,fullid:r})}).then(e=>e.json()).then(n=>{console.log("The room "+e+" added the participant:",t)})}async function ed(){let e=localStorage.getItem("name"),t=window.location.pathname.split("/"),n=t[t.length-1],r=(await ep(n)).idFull;await fetch(es+"/chat/"+n+"/removeparticipant",{method:"PUT",headers:{"Content-Type":"application/json"},body:JSON.stringify({name:e,idShort:n,fullid:r})}),console.log("The room "+n+" removed the participant:",e),await eh(n)}async function eh(e){let t=await ep(e),n=t.chatroom.participants?Object.keys(t.chatroom.participants).length:0,r=t.idFull;1==n&&fetch(es+"/chat/"+e+"/deletechatroom",{method:"PUT",headers:{"Content-Type":"application/json"},body:JSON.stringify({idShort:e,fullid:r})}).then(e=>e.json()).then(t=>{console.log("The room "+e+" is deleted")})}async function ep(e){try{let t=await fetch(es+"/chat/"+e,{method:"GET",headers:{"Content-Type":"application/json"}});if(!t.ok)throw Error("Error en la solicitud");return await t.json()}catch(e){return console.log("ERROR at listening room"),null}}function eu(){em("22338437"),em("4f94d8d9"),em("96479775")}async function em(e){let t=await ep(e);t.idFull;let n=Object.keys(t.chatroom.participants).length-1;if("22338437"==e){let e=document.querySelector(".pc1-participants");e&&(e.textContent=JSON.stringify(n))}else if("4f94d8d9"==e){let e=document.querySelector(".pc2-participants");e&&(e.textContent=JSON.stringify(n))}else if("96479775"==e){let e=document.querySelector(".pc3-participants");e&&(e.textContent=JSON.stringify(n))}return n}async function ef(e,t){let n=window.location.pathname.split("/"),r=n[n.length-1],o=Date.now(),i=localStorage.getItem("color"),a=(await ep(r)).idFull;fetch(es+`/chat/${r}/writemessages`,{method:"PUT",headers:{"Content-Type":"application/json"},body:JSON.stringify({idShort:r,fullid:a,user:e,message:t,timestamp:o,color:i})}).then(e=>e.json()).then(e=>{console.log("The message has been written.")})}async function eg(e){let t=(await ep(e)).idFull;console.log("the username is:",localStorage.getItem("name"));let n=new EventSource(`${es}/chat/${e}/listenmessages?fullid=${t}`);n.onmessage=e=>{let t=JSON.parse(e.data);null!=t.messages&&(console.log("Messages updated:",t.messages),function(e){let t=document.querySelector(".messages-container");t.innerHTML="";let n="",r=0;Object.entries(e).forEach(([e,o])=>{if("0"!=e){let e=document.createElement("div");if(e.classList.add("message"),o.username==localStorage.getItem("name")&&o.color==localStorage.getItem("color"))var i="admin";else var i="user";e.classList.add(`${i}`),e.textContent=o.message;let a=document.createElement("p");a.classList.add("user-msg"),a.classList.add(`${i}`);let s=o.color;null!=s&&(a.style.color=s),"user"==i&&n!=o.username&&(a.textContent=o.username),t.appendChild(a),t.appendChild(e),n=o.username,r+=1}}),t&&(t.scrollTop=t.scrollHeight)}(t.messages))},n.onerror=e=>{console.error("Error in the SSE:",e),n.close()}}const ev=window.location.pathname.split("/"),ex=ev[ev.length-1],eb="/live-chat";if(window.addEventListener("popstate",e=>{console.log("The URL changed to: ",window.location.pathname);let t=window.location.pathname.split("/"),n=t[t.length-1];window.location.pathname.startsWith(eb+"/chatrooms")&&(eu(),ec(),setInterval(()=>{window.location.pathname.startsWith(eb+"/chatrooms")&&eu()},3e3)),window.location.pathname.startsWith(eb+"/chat/"+n)&&window.addEventListener("DOMContentLoaded",()=>{el(n)}),window.location.pathname.startsWith(eb+"/chat/")&&eg(n)}),window.location.pathname.startsWith(eb+"/chatrooms")){let e=localStorage.getItem("name"),t=localStorage.getItem("color");e||t||X.go(eb+"/"),eu(),ec(),setInterval(()=>{window.location.pathname.startsWith(eb+"/chatrooms")&&eu()},3e3)}if(window.location.pathname.startsWith(eb+"/chat/"+ex)){let e=localStorage.getItem("name"),t=localStorage.getItem("color");e||t||X.go(eb+"/"),window.addEventListener("DOMContentLoaded",()=>{el(ex)})}document.addEventListener("submit",async e=>{if(e.target.classList.contains("form-send-msg")){e.preventDefault();let t=document.querySelector(".input-send-msg"),n=t.value;ef(localStorage.getItem("name"),n),t.value=""}}),document.addEventListener("keydown",function(e){if(e.target.classList.contains("input-send-msg")){let t=document.querySelector(".input-send-msg"),n=t.value,r=localStorage.getItem("name");r&&"Enter"===e.key&&!e.shiftKey&&(e.preventDefault(),ef(r,n),t.value="")}}),window.location.pathname.startsWith(eb+"/chat/")&&eg(ex),document.addEventListener("click",async e=>{let t=e.target;if(t.classList.contains("create_private"))e.preventDefault(),fetch(es+"/createchat",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({name:localStorage.getItem("name"),color:localStorage.getItem("color")})}).then(e=>e.json()).then(e=>{console.log("Room created with ID:",e.idShort),X.go("/live-chat/chat/"+e.idShort)});else if(t.classList.contains("back-button"))e.preventDefault(),ed(),X.go(eb+"/chatrooms");else if(t.classList.contains("public_chat_1")){e.preventDefault();let t=document.querySelector(".pc1-participants");if(t&&t.textContent){let e=parseInt(t.textContent,10);if(!isNaN(e)&&e<=10){let e="22338437";X.go(eb+"/chat/"+e),el(e)}}}else if(t.classList.contains("public_chat_2")){e.preventDefault();let t=document.querySelector(".pc2-participants");if(t&&t.textContent){let e=parseInt(t.textContent,10);if(!isNaN(e)&&e<=10){let e="4f94d8d9";X.go(eb+"/chat/"+e),el(e)}}}else if(t.classList.contains("public_chat_3")){e.preventDefault();let t=document.querySelector(".pc3-participants");if(t&&t.textContent){let e=parseInt(t.textContent,10);if(!isNaN(e)&&e<=10){let e="96479775";X.go(eb+"/chat/"+e),el(e)}}}else if(t.classList.contains("join_private"))e.preventDefault(),document.querySelector(".popup-container").innerHTML=`
