...
>>>>>>> 6f8fab9ee20e9c26d707603155d0e2565f492358
    ...
    ${this.firstVisit()?`<form class="form">
        ...
        </div>`}`;let t=this.container.querySelector(".form");t&&t?.addEventListener("submit",t=>{t.preventDefault();let n=t.target.elements.name.value.trim();this.containsBadWords(n)||""===n?(this.warning='<p class="warning">Please, use other words as a name.</p>',this.render()):(localStorage.setItem("name",n),localStorage.setItem("color",this.getNiceRandomColor()),this.name=n,X.go(e+"/chatrooms"))});let n=this.container.querySelector(".to-chatrooms");n&&n.addEventListener("click",t=>{t.preventDefault(),X.go(e+"/chatrooms")});let r=this.container.querySelector(".change-name");r&&r?.addEventListener("click",e=>{e.preventDefault(),e.target,localStorage.removeItem("name"),this.name=null,this.render()})}}customElements.define("home-page",ee);var et={};et=new URL("person-icon.82649727.png",import.meta.url).toString();class en extends HTMLElement{constructor(){super()}connectedCallback(){this.render()}render(){this.innerHTML=`
    <style>
    ...
    `;let t=this.querySelector(".back-home-button");t?.addEventListener("click",e=>{e.preventDefault(),e.target,X.go("/live-chat"+"/")})}}customElements.define("chatrooms-page",en);var er={};er=new URL("send-icon2.da343077.png",import.meta.url).toString();class eo extends HTMLElement{constructor(){super()}connectedCallback(){this.render()}render(){let t=window.location.pathname.split("/"),n=t[t.length-1];this.innerHTML=`
    <style>
    ...
    `}}customElements.define("chats-page",eo);const ei="/live-chat";new X(document.querySelector(".root")).setRoutes([{path:ei+"/",component:"home-page"},{path:ei+"/chatrooms",component:"chatrooms-page"},{path:ei+"/chat/:id",component:"chats-page"}]);const ea="https://backend-live-chat.onrender.com";function es(){fetch(ea+"/checkrooms",{method:"PUT",headers:{"Content-Type":"application/json"},body:JSON.stringify({username:localStorage.getItem("name"),color:localStorage.getItem("color")})}).then(e=>e.json()).then(e=>{console.log(e)})}async function ec(e){let t=localStorage.getItem("name"),n=localStorage.getItem("color"),r=(await eh(e)).idFull;fetch(ea+"/chat/"+e+"/addparticipant",{method:"PUT",headers:{"Content-Type":"application/json"},body:JSON.stringify({name:t,color:n,idShort:e,fullid:r})}).then(e=>e.json()).then(n=>{console.log("The room "+e+" added the participant:",t)})}async function el(){let e=localStorage.getItem("name"),t=window.location.pathname.split("/"),n=t[t.length-1],r=(await eh(n)).idFull;await fetch(ea+"/chat/"+n+"/removeparticipant",{method:"PUT",headers:{"Content-Type":"application/json"},body:JSON.stringify({name:e,idShort:n,fullid:r})}),console.log("The room "+n+" removed the participant:",e),await ed(n)}async function ed(e){let t=await eh(e),n=t.chatroom.participants?Object.keys(t.chatroom.participants).length:0,r=t.idFull;1==n&&fetch(ea+"/chat/"+e+"/deletechatroom",{method:"PUT",headers:{"Content-Type":"application/json"},body:JSON.stringify({idShort:e,fullid:r})}).then(e=>e.json()).then(t=>{console.log("The room "+e+" is deleted")})}async function eh(e){try{let t=await fetch(ea+"/chat/"+e,{method:"GET",headers:{"Content-Type":"application/json"}});if(!t.ok)throw Error("Error en la solicitud");return await t.json()}catch(e){return console.log("ERROR at listening room"),null}}function ep(){eu("22338437"),eu("4f94d8d9"),eu("96479775")}async function eu(e){let t=await eh(e);t.idFull;let n=Object.keys(t.chatroom.participants).length-1;if("22338437"==e){let e=document.querySelector(".pc1-participants");e&&(e.textContent=JSON.stringify(n))}else if("4f94d8d9"==e){let e=document.querySelector(".pc2-participants");e&&(e.textContent=JSON.stringify(n))}else if("96479775"==e){let e=document.querySelector(".pc3-participants");e&&(e.textContent=JSON.stringify(n))}return n}async function em(e,t){let n=window.location.pathname.split("/"),r=n[n.length-1],o=Date.now(),i=localStorage.getItem("color"),a=(await eh(r)).idFull;fetch(ea+`/chat/${r}/writemessages`,{method:"PUT",headers:{"Content-Type":"application/json"},body:JSON.stringify({idShort:r,fullid:a,user:e,message:t,timestamp:o,color:i})}).then(e=>e.json()).then(e=>{console.log("The message has been written.")})}async function ef(e){let t=(await eh(e)).idFull;console.log("the username is:",localStorage.getItem("name"));let n=new EventSource(`${ea}/chat/${e}/listenmessages?fullid=${t}`);n.onmessage=e=>{let t=JSON.parse(e.data);null!=t.messages&&(console.log("Messages updated:",t.messages),function(e){let t=document.querySelector(".messages-container");t.innerHTML="";let n="",r=0;Object.entries(e).forEach(([e,o])=>{if("0"!=e){let e=document.createElement("div");if(e.classList.add("message"),o.username==localStorage.getItem("name")&&o.color==localStorage.getItem("color"))var i="admin";else var i="user";e.classList.add(`${i}`),e.textContent=o.message;let a=document.createElement("p");a.classList.add("user-msg"),a.classList.add(`${i}`);let s=o.color;null!=s&&(a.style.color=s),"user"==i&&n!=o.username&&(a.textContent=o.username),t.appendChild(a),t.appendChild(e),n=o.username,r+=1}}),t&&(t.scrollTop=t.scrollHeight)}(t.messages))},n.onerror=e=>{console.error("Error in the SSE:",e),n.close()}}const eg=window.location.pathname.split("/"),ev=eg[eg.length-1],ex="/live-chat";if(window.addEventListener("popstate",e=>{console.log("The URL changed to: ",window.location.pathname);let t=window.location.pathname.split("/"),n=t[t.length-1];window.location.pathname.startsWith(ex+"/chatrooms")&&(ep(),es(),setInterval(()=>{window.location.pathname.startsWith(ex+"/chatrooms")&&ep()},3e3)),window.location.pathname.startsWith(ex+"/chat/"+n)&&window.addEventListener("DOMContentLoaded",()=>{ec(n)}),window.location.pathname.startsWith(ex+"/chat/")&&ef(n)}),window.location.pathname.startsWith(ex+"/chatrooms")){let e=localStorage.getItem("name"),t=localStorage.getItem("color");e||t||X.go(ex+"/"),ep(),es(),setInterval(()=>{window.location.pathname.startsWith(ex+"/chatrooms")&&ep()},3e3)}if(window.location.pathname.startsWith(ex+"/chat/"+ev)){let e=localStorage.getItem("name"),t=localStorage.getItem("color");e||t||X.go(ex+"/"),window.addEventListener("DOMContentLoaded",()=>{ec(ev)})}document.addEventListener("submit",async e=>{if(e.target.classList.contains("form-send-msg")){e.preventDefault();let t=document.querySelector(".input-send-msg"),n=t.value;em(localStorage.getItem("name"),n),t.value=""}}),document.addEventListener("keydown",function(e){if(e.target.classList.contains("input-send-msg")){let t=document.querySelector(".input-send-msg"),n=t.value,r=localStorage.getItem("name");r&&"Enter"===e.key&&!e.shiftKey&&(e.preventDefault(),em(r,n),t.value="")}}),window.location.pathname.startsWith(ex+"/chat/")&&ef(ev),document.addEventListener("click",async e=>{let t=e.target;if(t.classList.contains("create_private"))e.preventDefault(),fetch(ea+"/createchat",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({name:localStorage.getItem("name"),color:localStorage.getItem("color")})}).then(e=>e.json()).then(e=>{console.log("Room created with ID:",e.idShort),X.go("/live-chat/chat/"+e.idShort)});else if(t.classList.contains("back-button"))e.preventDefault(),el(),X.go(ex+"/chatrooms");else if(t.classList.contains("public_chat_1")){e.preventDefault();let t=document.querySelector(".pc1-participants");if(t&&t.textContent){let e=parseInt(t.textContent,10);if(!isNaN(e)&&e<=10){let e="22338437";X.go(ex+"/chat/"+e),ec(e)}}}else if(t.classList.contains("public_chat_2")){e.preventDefault();let t=document.querySelector(".pc2-participants");if(t&&t.textContent){let e=parseInt(t.textContent,10);if(!isNaN(e)&&e<=10){let e="4f94d8d9";X.go(ex+"/chat/"+e),ec(e)}}}else if(t.classList.contains("public_chat_3")){e.preventDefault();let t=document.querySelector(".pc3-participants");if(t&&t.textContent){let e=parseInt(t.textContent,10);if(!isNaN(e)&&e<=10){let e="96479775";X.go(ex+"/chat/"+e),ec(e)}}}else if(t.classList.contains("join_private"))e.preventDefault(),document.querySelector(".popup-container").innerHTML=`
